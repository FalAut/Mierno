...
- 修复基础机械白雏菊的配方缺失问题
- 调整梦之灯右键返回重生点所需的抬头幅度
- 修复黑莲花和暗黑莲花的稀有度设定缺失问题（感谢 @FutureLin-JM）
- 修复巨型熔炉的外部交互有时会与期望情况不一致的问题

---

- Fixed the issue of the basic mechanical white daisy's recipe being missing.
- Adjusted the upward angle required to return to the respawn point by right-clicking on the Dream Lantern.
- Fixed the problem of the rarity settings for the Black Lotus and Dark Lotus being missing (thanks @FutureLin-JM).
- Fixed the issue where the external interaction of the Colossal Furnace sometimes does not match the expected situation.
...
- 钢粒改为使用物质球合成
- 灌注室现在不会消耗基座上的物品
- 修复巨型熔炉中部分燃料会自己烧自己的问题
- 增加了迷宫的生成间隔，希望能修复有很小的概率会生成 2 个连在一起的迷宫的问题
- 删除了胶囊（改为使用多方块建造工具）
- 删除了植物魔法模块化（改为植物机械模组）
- 简化了一些前期配方，减少使用魔法设备的配方
- 用于转换液态魔力和液体魔源的储罐改为陨石储罐
